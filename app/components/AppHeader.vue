<template>
  <div>
      ...
      <div class="flex justify-between items-center border-b-2 border-gray-100 py-6 md:justify-start md:space-x-10">
          ...
          <div v-for="(item, index) in menu" :key="index">
            <div v-if="item.type === 'dropdown'" class="relative">
              ...
              <div
                ...
                <div
                  v-show="menuCollapse[item.status]"
        ...
        <div class="-mr-2 -my-2 md:hidden">
          <button
            ...
            type="button"
            @click="menuCollapse.mobile_menu = true"
          >
    ...
    <div
      v-show="menuCollapse.mobile_menu"
      class="absolute top-0 inset-x-0 p-2 transition transform origin-top-right md:hidden">
      <div class="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 bg-white divide-y-2 divide-gray-50">
        <div class="pt-5 pb-6 px-5">
          <div class="flex items-center justify-between" @click="menuCollapse.mobile_menu = false">
            <div>
              <h1 class="flex-auto text-lg font-semibold text-gray-900 sm:hidden">
                {{ title.short }}
              </h1>
...
const menuCollapse = ref<Record<string, boolean>>({
  mobile_menu: false
})
...
const menu = computed(() => [
  {
    ...
    action: () => {
      router.push('/join')
      menuCollapse.value.mobile_menu = false
    }
  },
  {
    name: "社群規範",
...
const openMenu = (item: string) => {
  Object.keys(menuCollapse.value).forEach(key => {
    if (key === 'mobile_menu') return
    if (key === item) {
      menuCollapse.value[key] = !menuCollapse.value[key]
    } else {
      menuCollapse.value[key] = false
    }
  })
}
